    ...
    while count_frame <= 3000 :
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,results.multi_handedness):
                ...
                if point != None : 
                    write_on_csv(point, pre_processed_landmark_list)    
                    count_frame = count_frame + 1
                    print(f"[DEBUG] count frame : {count_frame}")
                elif 48 <= key <= 53:  # 0 ~ 5 (6 gestures)
                    numb_class = key - 48    
...
def write_on_csv(number, landmark_list):
...
# alternative way to draw landmarks using openCV's circle and line
...
def get_key_point():
    ...
    point = args.point
    print(f"[DEBUG] point selected : {point}")
    return point
